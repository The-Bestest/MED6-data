...
```{r Parittion data}
discrete_label <- "Discrete" # TODO: fix the label
continuous_label <- "Continuous" # TODO: fix the label
...
```{r Check parametric data for normality grouped by order number and condition}
...
normalityCheck <- gameSummary %>%
  select(ObjectiveSuccesses, PerceivedSuccesses, ReportedAvgAttempts, Condition, orderNumber, Age) %>%
...
```
```{r Check parametric data for normality grouped by Sex}
# Check performance (accPerc), Perceived Successes, ReportedAvgAttempts for normality
# We don't need to check for variance with Levene's test, as Welch's test takes care of that.
# https://daniellakens.blogspot.com/2015/01/always-use-welchs-t-test-instead-of.html
...
# method from: https://sebastiansauer.github.io/multiple-t-tests-with-dplyr/
```
```{r Counter balancing for parametric data grouped by Sex - Randomization check}
...
sexCheck <- gameSummary %>%
  ...
  group_by(variable) %>%
  mutate(
    p_value   = wilcox.test(unlist(m), unlist(f), paired=FALSE, exact=FALSE)$p.value,
      statistic = wilcox.test(unlist(m), unlist(f), paired=FALSE, exact=FALSE)$statistic
  )
# method from: https://sebastiansauer.github.io/multiple-t-tests-with-dplyr/
```
...
filter_out = c(1,14,16,30,31) # columns to filter out
