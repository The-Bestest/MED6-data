...
cor.test(gameSummary$Frustration, gameSummary$ObjectiveFailures, method="spearman", exact = FALSE)
...
plotCorrelation(gameSummary, gameSummary$Frustration, gameSummary$SoA, "Frustration",  "SoA")
...
# Random sampling 
samplesize = 0.60*nrow(Discrete)
set.seed(100)
index = sample(seq_len(nrow(Discrete)), size = samplesize)
# Creating training and test set 
datatrain = Discrete[index,]
datatest = Discrete[-index,]

# Using polr for ordinal regression, as SoA is Likert Scale (no need to treat the data as nominal´)
model <- polr(factor(SoA, levels = likertScale, ordered = TRUE) ~ ObjectiveSuccesses, data = gameSummary, Hess = TRUE)

predictSoA = predict(model,gameSummary)
summary(model)
table(predictSoA, gameSummary$ObjectiveSuccesses)
mean(as.character(gameSummary$SoA) != as.character(predictSoA))
```
