...
# Is the date in each order parametric? YES if p > 0.05
shapiro.test(ParticipantOrder1$accPerc)$p.value
shapiro.test(ParticipantOrder2$accPerc)$p.value

shapiro.test(ParticipantOrder1$PerceivedSuccesses)$p.value
shapiro.test(ParticipantOrder2$PerceivedSuccesses)$p.value

shapiro.test(ParticipantOrder1$ReportedAvgAttempts)$p.value
shapiro.test(ParticipantOrder2$ReportedAvgAttempts)$p.value
```
```{r Counter balancing for performance - Randomization check}
# Does it matter for performance if discrete first or not?
t.test(ParticipantOrder1$accPerc, ParticipantOrder2$accPerc)
# Does it matter for performance in discrete condition if discrete first or not?
t.test(Discrete1$accPerc, Discrete2$accPerc)
# Does it matter for performance in continuous condition if continuous first or not?
t.test(Continuous1$accPerc, Continuous2$accPerc)

# Does it matter for perceived performance if discrete first or not?
t.test(ParticipantOrder1$PerceivedSuccesses, ParticipantOrder2$PerceivedSuccesses)
# Does it matter for perceived performance in discrete condition if discrete first or not?
t.test(Discrete1$PerceivedSuccesses, Discrete2$PerceivedSuccesses)
# Does it matter for perceived performance in continuous condition if continuous first or not?
t.test(Continuous1$PerceivedSuccesses, Continuous2$PerceivedSuccesses)

# Does it matter for reported attempts if discrete first or not?
t.test(ParticipantOrder1$ReportedAvgAttempts, ParticipantOrder2$ReportedAvgAttempts)
# Does it matter for reported attempts in discrete condition if discrete first or not?
t.test(Discrete1$ReportedAvgAttempts, Discrete2$ReportedAvgAttempts)
# Does it matter for reported attempts in continuous condition if continuous first or not?
t.test(Continuous1$ReportedAvgAttempts, Continuous2$ReportedAvgAttempts)
```
...
res2 <- Hmisc::rcorr(as.matrix(gameSummary[,-filter_out]))
...
Investigate the relationships closer.
```{r Investigate relationships}
# Plot of SoA vs SoO, per-condition
# TODO: make it possible to look at multiple variable pairs
gameSummary %>%
