...
normalityCheck <- gameSummary %>%
  select(accPerc, PerceivedSuccesses, ReportedAvgAttempts, Condition, orderNumber, Age) %>%
...
normalitySexCheck <- gameSummary %>%
  select(accPerc, PerceivedSuccesses, ReportedAvgAttempts, Sex) %>%
...
#We need to filter the followwing columns: Participant, Condition, Order number, activate delay, Sex and BCI 
filter_out = c(1,2,13,15,29,31) # columns to filter out
...
performanceCheck <- gameSummary %>%
  select(Condition, accPerc, PerceivedSuccesses, ReportedAvgAttempts) %>%
  gather(key = variable, value = value, -Condition) %>%
  group_by(Condition, variable) %>%
  summarise(value=list(value)) %>%
  spread(Condition, value) %>%
  ...
  mutate(
    # TODO: Rename Body and Blocks to actual condition names once we have new data
    p_value = t.test(unlist(Body), unlist(Blocks))$p.value,
    statistic = t.test(unlist(Body), unlist(Blocks))$statistic
  )
```
